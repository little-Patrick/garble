...
// same CHARACTERS set you had (ASCII-friendly)
...
/// Example quick test in a `main` function:
#[cfg(test)]
mod tests {
    use super::*;
    ...
    #[test]
    fn smoke() {
        let out = poly_cipher("Password123", "12345").expect("should succeed");
        // just assert it runs; exact output will depend on your shift math
        assert!(!out.is_empty());
    }

    #[test]
    fn bad_pin() {
        assert!(matches!(poly_cipher("x", "12"), Err(CipherError::InvalidPinLength)));
    }
}
